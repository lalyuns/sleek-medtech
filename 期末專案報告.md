...
這樣設計後，管理者可以從同一個 BOM 畫面確認：這是哪一個器材、用在哪個部位、對應哪一版 3D 模型、用了哪些材料與組件，以及該版本的總成本與毛利估算。
...
系統將變動成本與固定租金攤提結合，自動生成單一專案的 BOM 表與成本摘要。管理者可一鍵查看每件客製化骨釘、手術輔助器材或模型的真實利潤率，並同時確認該成本對應的臨床用途與版本脈絡。
...
3. 器材用途關聯：`projects.product_id -> products` 會讓 BOM API 一併回傳 `body_region`、`clinical_use`、`surgical_stage`、`indication` 與目前 `model_version`，因此前端可直接顯示「這個器材用在哪個部位、對應哪一版 STL、用了哪些材料、成本是多少」。
...
| 成本與毛利不清楚 | BOM 與成本資料庫 | 集中記錄材料、工時、打樣、報價，並把成本連到器材用途、身體部位與 STL 版本。 |
...
本節截圖皆來自實際啟動本專案後的操作畫面。展示流程依照使用者角色安排，先確認後端服務與 API，再依序展示外部申請人、系統管理員、研發工程師、醫師與廠商的操作情境。

### 1. 啟動後端與 demo 測資

![後端控制台](demo-assets/00-backend-console.png)

系統以 Docker Compose 啟動後，包含後端、MySQL、Redis、MinIO 等服務。後端會執行 Alembic migration，並可透過 `reset_demo_data.py` 建立 demo 使用者、專案、材料、產品型錄、STL 版本、BOM 與報告資料。這一步用來確認助教或使用者 clone 專案後，能在本機重建展示環境。

### 2. 後端 API 文件與服務確認

![OpenAPI](demo-assets/01-backend-openapi.png)

FastAPI 會自動產生 OpenAPI 文件。此畫面用來確認後端 API 已正常啟動，也能看到系統目前提供的 REST API 端點。前端所有專案、產品、使用者、材料、BOM、報告與稽核功能，都是透過這些 API 與後端協作。

### 3. 外部申請人：公開產品型錄

![公開型錄](demo-assets/02-public-catalog.png)

外部訪客不需要登入，即可進入公開產品型錄查看可申請產品。畫面中會呈現產品或套組名稱、說明、使用部位、臨床用途、適應症、組件來源、BOM 數量與文件需求。使用者也能用關鍵字搜尋，或點選相似部位/用途/情境標籤反查相關器材。這對應到公司「沒有專門業務人員」的痛點，讓網站先成為基本對外展示入口。

![公開申請結果](demo-assets/03-public-request-result.png)

外部訪客填寫姓名、單位、Email、電話、數量與補充說明後，可以送出需求申請。系統只建立申請紀錄，不會自動開通內部帳號，後續由管理員在內部產品管理頁審核與追蹤。
...
管理員登入後可看到所有專案與專案統計。專案列表也會顯示是否連結產品套組，讓內部專案能同時管理 STL 模型版本與產品 BOM。
...
公開型錄送出的需求會進入外部申請列表。管理員可以查看申請人、單位、聯絡方式、需求數量與狀態，並將申請狀態改為審核中、已報價、已核准或已拒絕。
...
使用者管理頁可建立或刪除內部帳號，角色包含管理員、工程師、醫師與廠商。此功能支援公司在未來擴張時管理不同協作對象。

![稽核紀錄](demo-assets/10-admin-audit.png)

稽核頁會記錄建立、更新、刪除、上傳、簽核與材料變更等操作。當面臨法規稽核或內部追蹤時，可以回查誰在什麼時間對哪筆資料做了什麼事。

### 6. 研發工程師：STL 上傳與 BOM 成本

![工程師上傳](demo-assets/11-engineer-upload.png)

工程師進入專案後，可以在上傳分頁選擇材料與 STL 檔案。系統會透過上傳流程建立版本，不允許手動建立沒有檔案、hash 或體積資料的假版本。

![工程師 BOM](demo-assets/12-engineer-bom.png)

BOM 分頁會顯示 STL 材料成本、工時或外部打樣成本，也會顯示專案連結的產品套組零件 BOM。新版 BOM 摘要會在最上方顯示器材名稱、使用部位、臨床用途、使用階段、適應症與目前 STL 版本，因此可以直接看出「這個成本是用在哪個器材、哪個身體部位與哪一版模型」。這對應到公司目前成本分散、難以計算單案真實毛利的痛點。

### 7. 醫師：3D 檢視與版本回饋

![醫師回饋](demo-assets/13-doctor-review-feedback.png)

醫師登入後可進入授權專案，在 3D 檢視中查看模型，並針對特定版本留下文字回饋或座標註記。這些回饋會被系統保存，後續版本可引用回饋作為修改依據，形成可追蹤的設計決策鏈。

### 8. 廠商：唯讀協作

![廠商唯讀總覽](demo-assets/14-vendor-readonly-project.png)

廠商登入後只會看到被授權的專案資料，預設為唯讀。系統不會顯示上傳、成本輸入、成員管理或簽核等內部操作，避免外部協作者誤改核心資料。

![廠商 3D 檢視](demo-assets/15-vendor-3d-view.png)

廠商可查看模型、版本與基本專案資訊，用於理解製作需求或對接打樣流程。這讓外部廠商能參與協作，但權限仍受控於系統。


### 9. 完整 Demo 腳本與操作帳號

以下內容由原 demo 文件逐項併入正文，補足啟動指令、角色帳號、操作流程、預期結果與展示截圖說明。
