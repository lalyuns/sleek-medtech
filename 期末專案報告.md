...
第二個是 `products`，負責對外產品與套組管理。`products` 可以連到 `projects`，表示某個內部專案對應哪個產品套組；`products` 也會透過 `product_bom_items` 連到 `components`，形成產品 BOM。外部訪客在公開型錄送出的需求會存成 `product_requests`，並連回指定產品。`audit_logs` 則記錄使用者對各種實體的操作紀錄。
