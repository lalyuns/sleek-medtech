...
此功能用來解決「系統功能很多，但第一次使用者不知道從哪裡開始」的問題。醫材協作平台本身包含專案、版本、材料、報告、BOM、產品、外部申請、成員權限與稽核紀錄，若沒有導覽，新同仁或展示對象容易被資訊量淹沒。

#### 具體作法

系統新增 `/guide` 新手導覽頁，將操作路徑依角色拆成管理員、研發工程師、醫師審閱與業務/對外窗口四種情境。使用者不需要先理解所有資料表與功能，只要選擇自己的角色，就能看到：

- 這個角色的主要目標。
- 進系統後第一步要看哪裡。
- 第二步與第三步要補什麼資料或完成什麼操作。
- 對應的系統入口，例如公開型錄、登入、專案管理或產品管理。

#### AI 輔助定位

AI 不直接做醫療決策，而是用來整理資訊與降低交接成本。系統在導覽頁展示 AI 交接摘要的使用方式，未來可串接真實資料後自動產生：
...
這樣設計可以讓公司合理投入 AI 預算，因為 AI 的價值不是抽象的聊天功能，而是直接幫公司節省交接、客服、業務說明與內部追蹤成本。

#### 具體成效

- 新人坐下來即可依角色進入正確流程。
- Demo 時不需要口頭解釋每一個頁面，導覽本身就能說明系統價值。
- 管理者可把 AI 摘要當作每日交接與追蹤清單。
- 對外可強化「公司正在導入 AI 營運助理」的形象，讓投資人看到技術公司願意把 AI 用在真實營運問題上。
...
系統有兩個核心主軸。第一個是 `projects`，負責內部醫材協作；一個專案會有多個 `model_versions`、`reports`、`costs`，也會透過 `user_project_mappings` 控制哪些使用者可以存取。`model_versions` 會連到 `materials`，表示每一版 STL 使用哪個材料；也會連到 `feedbacks`，表示醫師或授權使用者對某一版模型留下回饋。若某一版模型參考了舊版、回饋或報告，則用 `reference_edges` 建立 traceability graph。

第二個是 `products`，負責對外產品與套組管理。`projects` 可以連到 `products`，表示某個內部專案對應哪個產品套組；`products` 也會透過 `product_bom_items` 連到 `components`，形成產品 BOM。外部訪客在公開型錄送出的需求會存成 `product_requests`，並連回指定產品。`audit_logs` 則記錄使用者對各種實體的操作紀錄。
...
| `target_type` | 關聯目標類型，可為 `old_version`、`feedback`、`report`。 |
| `target_id` | 對應目標表的主鍵。 |
| `relation_type` | 關聯語意，目前用於表示版本溯源、回饋依據或報告佐證。 |

`reference_edges` 採多型關聯設計，因此 Mermaid E-R 圖只畫出 `MODEL_VERSIONS` 到 `REFERENCE_EDGES` 的來源關係；`target_type + target_id` 會再指向舊版本、回饋或報告。
...
| `cost_id` | 成本 ID。 |
| `project_id` | 成本所屬專案。 |
| `cost_type` | 成本類型，目前為 `labor` 或 `external_sample`。 |
| `amount` | 成本金額。 |
| `note` | 成本說明，例如工時檢討、樣件列印、表面處理。 |
| `created_at` | 建立時間。 |
...
完整 API 文件由 FastAPI 依照後端程式自動產生，demo 環境可開啟 `http://localhost:8000/docs` 查看所有 endpoint、request body、query parameter 與 response schema；也可透過 `http://localhost:8000/openapi.json` 檢視 OpenAPI JSON。報告中不逐支貼出所有 API，而是整理核心 API 群組、代表端點與其支撐的使用流程。所有需認證 API 使用 `Authorization: Bearer <access_token>`。
...
| API 群組 | 代表端點 | 支撐的系統流程 |
| --- | --- | --- |
| 認證與授權 | `POST /api/v1/auth/login`、`POST /api/v1/auth/refresh` | 內部使用者登入、更新 token，後續 request 以 JWT 搭配 RBAC 與專案權限判斷可見資料。 |
| 公開產品型錄 | `GET /api/v1/catalog/products`、`POST /api/v1/catalog/requests` | 外部訪客不登入即可查詢公開產品、查看 BOM 摘要，並送出需求申請。 |
| 產品、組件與產品 BOM | `GET /api/v1/products`、`POST /api/v1/products/{product_id}/bom`、`GET /api/v1/product-requests` | 管理員維護對外產品、零件來源、套組 BOM，並追蹤外部申請狀態。 |
| 專案與成員 | `GET /api/v1/projects/`、`GET /api/v1/projects/{project_id}`、`POST /api/v1/projects/{project_id}/members` | 管理內部醫材專案、專案與產品套組的連結，以及工程師、醫師、廠商的專案權限。 |
| 材料與使用者管理 | `GET /api/v1/materials/`、`POST /api/v1/materials/`、`GET /api/v1/users/` | 維護 STL 成本計算所需的材料密度、單價與內部帳號角色。 |
| STL 版本上傳 | `POST /api/v1/projects/{project_id}/versions/upload/init`、`POST /api/v1/projects/{project_id}/versions/{version_id}/upload/complete`、`GET /api/v1/projects/{project_id}/versions/{version_id}/upload/status/{job_id}` | 由前端分塊上傳 STL，後端建立版本與背景解析工作，worker 完成 hash、體積與狀態更新。 |
| 3D 回饋與簽核 | `GET /api/v1/projects/{project_id}/versions`、`POST /api/v1/projects/{project_id}/versions/{version_id}/feedbacks`、`POST /api/v1/projects/{project_id}/versions/{version_id}/lock` | 醫師在特定模型版本留下回饋或 3D 座標註記，必要時執行版本簽核並鎖定。 |
| 報告與版本引用 | `GET /api/v1/projects/{project_id}/reports`、`POST /api/v1/projects/{project_id}/reports`、`POST /api/v1/projects/{project_id}/versions/{version_id}/references` | 上傳材料、檢驗、合規、供應商報價等文件，並將報告或回饋連回特定模型版本。 |
| BOM、成本與溯源 | `GET /api/v1/projects/{project_id}/versions/{version_id}/bom`、`POST /api/v1/projects/{project_id}/costs`、`GET /api/v1/projects/{project_id}/versions/{version_id}/traceability` | 整合 STL 材料成本、額外成本、產品套組 BOM 與版本證據鏈。 |
| 稽核紀錄 | `GET /api/v1/audit/logs`、`GET /api/v1/audit/logs/export.csv`、`GET /api/v1/audit/logs/export.pdf` | 查詢上傳、建立、更新、簽核等操作紀錄，支援法規稽核與內部追蹤。 |

產品、組件、BOM 編修、使用者管理與申請狀態更新需管理員權限；工程師可操作被授權專案的上傳、報告與成本；醫師可檢視、回饋與簽核；廠商預設為唯讀。建立專案時可傳 `product_id`，用來連結產品套組 BOM，後端會確認該產品存在、啟用且未刪除。

###### 4.2 BOM 與成本 API 設計重點
