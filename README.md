...
- 產品與套組管理：公開產品型錄、內部產品/組件/BOM 管理、外部需求申請，以及專案連結產品套組。
...
demo 專案 `MR-2026-041` 與 `CM-2026-017` 會透過 `projects.product_id` 連到對應產品套組，所以在專案 BOM 分頁可同時看到 STL 材料成本與套組零件 BOM。
...
- 專案 BOM 分成兩層：STL 材料/工時成本，以及產品套組的組件 BOM。
